...
UpdateNodes.prototype.build = function (protocol) {
    ...
    var partyin = this.playerTracker._partyindex;
    writer.writeUInt8(partyin);
...
UpdateNodes.prototype.writeUpdateItems4 = function (writer) {
    ...
    for (var i = 0; i < this.updNodes.length; i++) {
        ...
        //var isMuted = node.owner.isMuted ? 1 : 2;//ju0803

        // Write update record
        writer.writeUInt32((node.nodeId ^ scrambleId) >>> 0);  //set step 5       // Cell ID
        writer.writeUInt16(cellX >> 0);                // Coordinate X
        writer.writeUInt16(cellY >> 0);                // Coordinate Y
        writer.writeUInt16(node._size >>> 0);     // step 8  set size// Cell Size (not to be confused with mass, because mass = size*size/100)
        //writer.writeUInt8(isMuted);//ju0803
...
UpdateNodes.prototype.writeEatItems = function (writer) {
    ...
    writer.writeUInt16(this.eatNodes.length >>> 0);   //step 2 add eat nodes's count         // EatRecordCount
    for (var i = 0; i < this.eatNodes.length; i++) {
        ...
        writer.writeUInt32((hunterId ^ scrambleId) >>> 0);               // Hunter ID// step 3setkiller
        writer.writeUInt32((node.nodeId ^ scrambleId) >>> 0);            // Prey ID//steo 4 set killed
    }
};
