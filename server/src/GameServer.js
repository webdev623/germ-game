...
serverEndTime = 1800; // for Experimental Mode
...
function GameServer() {
    ...
    this.config = {
        ...
        serverPort: serverport, // Server port which will be used to listen for incoming connections
        ...
        serverGamemode: servergamemode, // Gamemodes: 0 = FFA, 1 = Teams, 2 = Experimental, 3 = Rainbow
        ...
        serverStatsPort: Number(String(serverport).substring(1)), // Port for stats server. Having a negative number will disable the stats server.
        serverStatsUpdate: 1, // Update interval of server stats in seconds
        ...
        serverGameTime:servergametime,// for Experimental Game Mode servergamemode = 2 
...
GameServer.prototype.start = function () {
    ...
    console.log("111111111111start");
    this.timerLoopBind = this.timerLoop.bind(this);
    this.mainLoopBind = this.mainLoop.bind(this);
    // Set up gamemode(s)
    var Gamemode = require('./gamemodes');
	
	console.log(this.config.serverGamemode);
...
GameServer.prototype.onHttpServerOpen = function () {
    Logger.write("111111111111http open");
    // Start Main Loop
...
GameServer.prototype.addNode = function (node) {
    //Logger.write("111111111111addNode");
...
GameServer.prototype.onClientSocketOpen = function (ws, req) {
    Logger.write("111111111111onClientSocketOpen");
    ...
    Logger.write("333333333333onClientSocketOpen");
    ws.isConnected = true;
    ws.remoteAddress = ws._socket.remoteAddress;
    ws.remotePort = ws._socket.remotePort;
    ws.lastAliveTime = Date.now();
    Logger.write("222222222222onClientSocketOpen");
...
GameServer.prototype.updateClients = function () {
    ...
    for (var i = 0; i < len;) {
        ...
        if (this.clients[i].playerTracker.isRemoved) {
            // remove dead client and leave party
            //console.log("Leave party and delete cell arrayyyyyyyyyyy");
			var pid = this.clients[i].playerTracker._partyindex;
			var ptracker = this.clients[i].playerTracker
            this.LeaveParty(pid, ptracker);
   ...
   //for (var i = 0; i < len; i++) { 
   //    if (!this.clients[i]) continue;
   //    if (this.clients[i].playerTracker._partyindex !== 1) {
   //        for (var j = 0; j < len; j++) {
   //            if (i == j) continue;
   //            if (this.clients[j] && this.clients[i].playerTracker._partyindex === this.clients[j].playerTracker._partyindex) {
   //                this.clients[i].playerTracker.getPosInfo(this.clients[j].playerTracker);
   //                this.clients[i].playerTracker.sendAddOther();
   //            }
   //        }
   //    }
   //}
 ...
 //              if (i == j) continue;
 //              if (this.clients[j] && this.clients[i].playerTracker._partyindex === this.clients[j].playerTracker._partyindex) {
 //                  this.clients[i].playerTracker.sendAddOther(this.clients[j].playerTracker);
 //                  //this.clients[j].playerTracker.sendAddOther(this.clients[i].playerTracker);
 //              }
 //          }
 //      }
 //  }
    ...
    for (var i = 0; i < len; i++) {
        ...
        var partyclients = [];
        for (var j = 0; j < len; j++) {
            if (this.clients[i] != 1 && i != j) {
                if (this.clients[i].playerTracker._partyindex == this.clients[j].playerTracker._partyindex) {
                    partyclients.push(this.clients[j]);
                }
            }
        }
        this.clients[i].playerTracker.partyclients = partyclients;
        */
...
GameServer.prototype.updatePartyTable = function () {
    for (var i = 0; i < this.clients.length; i++) {
        this.clients[i].playerTracker.updateParty(this.clients);
    }
};
...
GameServer.prototype.onChatMessage = function (from, to, message) {
    ...
    //console.log("chat message arrive here 666666666666666666" + from._name + " to >>>>>" + to + " : message ====== " + message);
    if (!message) return;
   ...
   //if (!this.config.serverChat || (from && from.isMuted && from.isMutedName == to.isMutedName)) {
...
GameServer.prototype.sendChatMessage = function (from, to, message) {
    if (from) {
        //console.log("chat message arrive here 55555555555555>>" + from.isMutedName + ":");
    }
    ...
    for (var i = 0, len = this.clients.length; i < len; i++) {
        if (!this.clients[i]) continue;
        //console.log(len);
        var realName = this.clients[i].playerTracker._name;
        realName = realName || "";
        var nameAndSkin = /^(?:\{([^}]*)\})?([^]*)/.exec(realName);
        realName = nameAndSkin[2].trim();
        
        if (from) {
            var str = from.isMutedName.substring(1, realName.length + 1);
            //console.log("Real name >>>>>>>" + realName + "ismuted option>>>>>" + from.isMuted + "<<<Realname>>>>>" + str);
        }
        
        if (!this.config.serverChat || (from && from.isMuted && str === realName)) {
            // chat is disabled or player is muted
            //console.log("chat is disabled or player is muted");
            continue;
        }
...
//  190731 by ju
GameServer.prototype.sendPartyRequest = function(sender, clientname){
    //console.log("send request");
    if (sender._partyindex == 1) {
        sender.changePartyIndex();
    }
    
    for (var i = 0, len = this.clients.length; i < len; i++){
        if (!this.clients[i]) continue;
        var Packet = require('./packet');
        this.clients[i].packetHandler.sendPacket(new Packet.AddPartyMem(sender, clientname));
    }
};

//  190731 by ju
GameServer.prototype.LeaveParty = function(index, sender){
    //console.log("Game server arrive leave request");
	len = this.clients.length; 
    for(var i = 0;  i < len; i++){
        var Packet = require('./packet');
        try{
            var x = this.clients[i].playerTracker._partyindex.toString();
            var y = index.toString();
            var lenn = x.length > y.length ? y.length : x.length;
            x = x.substring(0, lenn);
            y = y.substring(0, lenn);
            
            if(x ===y || this.clients[i].playerTracker._partyindex === index){
                this.clients[i].packetHandler.sendPacket(new Packet.LeaveParty(sender));
            }
        }
        catch(err){
            continue;
...
//  190731 by ju
GameServer.prototype.SetMute = function (setting, name, muteName) {
    
    var len2 = name.length;
    for(var i = 0, len = this.clients.length; i < len; i++){
        var Packet = require('./packet');
        var len1 = this.clients[i].playerTracker._name.length;
        //console.log(this.clients[i].playerTracker._name + ">>" + len1);
        var realname = this.clients[i].playerTracker._name.substring(len1, len1 - len2  + 1);
        //console.log("realname >> " + realname + " : name >> " + name);
        len1 = name.length > realname.length ? realname.length : name.length;

        if(name.substring(0, len1) === realname.substring(0, len1)){
            if (setting === 1) {
                //console.log("Is Muted here>>>>>" + muteName);
                this.clients[i].playerTracker.isMuted = true;
                this.clients[i].playerTracker.isMutedName = muteName;
            }
            else{
                this.clients[i].playerTracker.isMuted = false;
                this.clients[i].playerTracker.isMutedName = "";
            }
            this.clients[i].packetHandler.sendPacket(new Packet.ChangeMute(this.clients[i].playerTracker));
        }
    }
...
//  190731 by ju
GameServer.prototype.sendAcceptParty = function(accept, clientname, partyname){
    //console.log("Accept Arrive Game Server : accept : " + accept + " : clientname : " + clientname + " : partyname : " + partyname);

    for(var i = 0, len = this.clients.length; i < len; i++){
        if(!this.clients[i])continue;
        var Packet = require('./packet');
       
        if (this.clients[i].playerTracker._name == partyname) 
            console.log("find server>>>>>>>>>" + partyname);
        var x = this.clients[i].playerTracker._name;
        var y = partyname;
        ...
        if ( x===y || partyname.trim() == this.clients[i].playerTracker._name.trim())
        {
            console.log("neeee");
            if(accept === 1)
            {
                for(var j = 0, l = this.clients.length ; j < l ; j++){
                    if (clientname === this.clients[j].playerTracker._name) {
                        this.clients[j].playerTracker._partyindex = this.clients[i].playerTracker._partyindex;
                        break;
                    }
                }
            }
        }
        else if (this.clients[i].playerTracker._name.trim() === clientname.trim() && accept == 1)
        {
            for(var j = 0, l = this.clients.length ; j < l ; j++){
                var x = this.clients[j].playerTracker._name;
                var y = partyname;
                ...
                if ( x===y || partyname.trim() === this.clients[j].playerTracker._name) {
                    this.clients[i].playerTracker._partyindex = this.clients[j].playerTracker._partyindex;
                    break;
                }
            }
            console.log("oooo " + partyname );
            this.clients[i].packetHandler.sendPacket(new Packet.AcceptParty(accept, partyname, this.clients[i].playerTracker._partyindex));
        }
    }
};

GameServer.prototype.isValidPartyIndex = function(partyIndex){
    for(var i = 0, len = this.clients.length; i < len; i++){
        if(!this.clients[i]) continue;
        if(this.clients[i].playerTracker._partyindex === partyIndex)
            return true
    }
    return false;
}

GameServer.prototype.Getmember = function(name){
   
};

//  190731 by ju
//GameServer.prototype.sendCreateParty = function(sender){
//    //console.log("send party create and change index");
//    packetHandler.socket.playerTracker.sendPacket();
//    for(var i = 0, len = this.clients.length; i < len; i++){
//        if(!this.clients[i])continue;
//        var Packet = require('./packet');
//        this.clients[i].packetHandler.sendPacket(new Packet);//
//    }
//}
...
GameServer.prototype.mainLoop = function () {
    ...
    if (((this.tickCounter + 7) % 25) === 0)
        this.updatePartyTable();// once per second
...
GameServer.prototype.spawnCells = function () {
    Logger.write("spawnCells99999999999999");
